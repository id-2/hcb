...
module DisbursementService
  class Create
    ...
    def initialize(source_event_id:, destination_event_id:, name:, amount:,
                   requested_by_id:, fulfilled_by_id: nil, destination_subledger_id: nil,
                   scheduled_on: nil, source_subledger_id: nil, should_charge_fee: false,
                   skip_auto_approve: false, fronted: false, create_cpts: true)
    ...
    def run
      ...
      if @create_cpts
        ::DisbursementService::CreateCanonicalPendingTransactions.new(disbursement: disbursement, fronted: @fronted).run
      end
