...
class PartnerSandbox < Partner
  ...
  def validate_stripe_keys_are_test_mode
    ...
    if self.stripe_api_key.present? && !self.stripe_api_key.starts_with?(TEST_API_KEY_PREFIX)
      errors.add(:stripe_api_key, 'must be a stripe test api key')
    end

    if self.public_stripe_api_key.present? && !self.public_stripe_api_key.starts_with?(TEST_PUBLIC_API_KEY_PREFIX)
