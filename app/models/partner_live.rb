...
class PartnerLive < Partner
  ...
  def validate_stripe_keys_are_live_mode
    ...
    if self.stripe_api_key.present? and !self.stripe_api_key.starts_with? LIVE_API_KEY_PREFIX
      errors.add(:stripe_api_key, 'must be a stripe live api key')
    end

    if self.public_stripe_api_key.present? and !self.public_stripe_api_key.starts_with? LIVE_PUBLIC_API_KEY_PREFIX
